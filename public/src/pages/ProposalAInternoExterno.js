...
const ProposalAInternoExterno = () => {
  ...
  const modelempty = "2. Modelagem 3D externo, conforme projeto e necessidades do cliente; 2.1 - Paredes externas conforme planta baixa (croqui, dwg ou pdf) recebida pelo cliente; 2.2 - Terreno; 2.3 - Muros; 2.4 - Pátio completo, com todos as áreas previstas no projeto arquitetônico; 2.5 - Acessos; 2.6 - Paisagismo básico genérico; 2.7 - Entorno básico; 2.8 - Humanização básica;"
  const modeltype = "A";
  ...
  const renderServicos = () => {
    return (
        <div>
          <h3>Serviços A Interno & Externo:</h3>
          <label>
            <input
              type="checkbox"
          ...
          <label>
            <input
              type="checkbox"
              value="Planta baixa humanizada em .png"
              onChange={handleProjectServiceChange}
            />
            Planta baixa humanizada em .png
          </label>
          <label>
            <input
              type="checkbox"
...
{/* 
  ...
  try {
    const response = await api.post('/mvpproposals/ainextern', payload, { responseType: 'blob' });
  ...
  return (
    ...
    <div className="container">
        <h1>Criação de Proposta de Investimento | Modelo A Interno & Externo</h1>
...
export default ProposalAInternoExterno;
