...
const ProposalCInternoExterno = () => {
  ...
  const model = "1. Modelagem 3D interno, conforme projeto e necessidades do cliente; 1.1 - Desenvolvimento e reformulação de layout interno; 1.2 - Criação de mobiliário em MDF/Metal; 1.3 - Paredes internas (conforme medidas/croqui recebidas pelo cliente); 1.4 - Decoração básica; 1.5 - Humanização básica; 1.6 - Iluminação;  1.7 - Aberturas internas; 1.8 - Cenografia básica; 1.9 - Cenografia avançada;"
  const modelempty = "2. Modelagem 3D externo, conforme projeto e necessidades do cliente; 2.1 - Paredes externas (conforme medidas/croqui recebidas pelo cliente); 2.2 - Terreno; 2.3 - Entorno básico; 2.4 - Muros; 2.5 - Pátio completo, com todos as áreas previstas no projeto arquitetônico e vídeos/fotos do local; 2.6 - Acessos; 2.7 - Paisagismo básico genérico; 2.8 - Entorno básico; 2.9 -Humanização básica; 2.10 - Humanização básica; 2.11 - Cenografia básica; 2.12 - Cenografia avançada;"
  ...
  const renderServicos = () => {
    return (
        <div>
          <h3>Serviços C Interno & Externo:</h3>
          <label>
            <input
              type="checkbox"
          ...
          <label>
            <input
              type="checkbox"
              value="Planta baixa com medidas básicas, em pdf"
              onChange={handleProjectServiceChange}
            />
            Planta baixa com medidas básicas, em pdf
          </label>
          <label>
            <input
              type="checkbox"
              value="1 Vídeo interno/externo realista em FULL HD de 60seg."
              onChange={handleProjectServiceChange}
            />
            1 Vídeo interno/externo realista em FULL HD de 60seg.
          </label>
        </div>
      );
  };
  ...
  try {
    const response = await api.post('/mvpproposals/aextern', payload, { responseType: 'blob' });
  ...
  return (
    ...
    <div className="container">
        <h1>Criação de Proposta de Investimento | Modelo C Interno & Externo</h1>
...
export default ProposalCInternoExterno;
